/**
 * Tests for useBuyCollateral hook
 * 
 * Testing approach:
...
import { useBuyCollateral } from '../useBuyCollateral';
...
// Mock CometClient
const mockBuyCollateral = jest.fn().mockResolvedValue({
...
const mockCometContract = {
  buyCollateral: mockBuyCollateral,
};
...
describe('useBuyCollateral', () => {
  ...
  it('should buy collateral correctly', async () => {
    const client = new CometClient({} as any, 1);
    
    const { result } = renderHook(() => useBuyCollateral(client), { wrapper });
    ...
    // Call buyCollateral
    const asset = '0x0987654321098765432109876543210987654321';
    const minAmount = BigNumber.from('1000000000000000000'); // 1 token
    const baseAmount = BigNumber.from('2000000000000000000'); // 2 tokens
    const recipient = '0x1234567890123456789012345678901234567890';
    
    let txHash: string | null = null;
    await act(async () => {
      txHash = await result.current.buyCollateral(
        asset,
        minAmount,
        baseAmount,
        recipient
    ...
    expect(mockBuyCollateral).toHaveBeenCalledWith(
      asset,
      minAmount,
      baseAmount,
      recipient
