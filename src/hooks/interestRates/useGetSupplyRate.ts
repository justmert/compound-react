import { useState, useEffect, useCallback } from 'react';
import { BigNumber } from 'ethers';
import { CometClient } from '../../api/CometClient';
import { useGetUtilization } from './useGetUtilization';
import { useCompoundContext } from '../../context/CompoundContext';

/**
 * Interface for the supply rate hook return value
 */
export interface SupplyRateData {
  supplyRate: BigNumber | null;
  supplyRateAPR: number | null;
  supplyRateAPY: number | null;
  loading: boolean;
  error: Error | null;
  refetch: () => Promise<void>;
}

/**
 * Interface for the supply rate hook options
 */
export interface SupplyRateOptions {
  chainId?: number;
  customUtilization?: BigNumber;
}

/**
 * Hook to get the current supply interest rate from Compound III
 * 
 * The supply rate is the interest rate earned by suppliers of the base asset.
 * It's determined by the utilization rate and the interest rate model parameters.
 * 
 * @param clientOrOptions - Optional CometClient instance or options
 * @param customUtilization - Optional custom utilization rate to calculate supply rate for
 * @returns Object containing supply rate data, loading state, error, and refetch function
 */
export function useGetSupplyRate(
  clientOrOptions?: CometClient | SupplyRateOptions,
  customUtilization?: BigNumber
): SupplyRateData {
  const context = useCompoundContext();
  
  // Determine which client to use and extract options
  let client: CometClient | null;
  let utilizationOverride: BigNumber | undefined;
  
  if (clientOrOptions instanceof CometClient) {
    client = clientOrOptions;
    utilizationOverride = customUtilization;
  } else if (clientOrOptions && 'customUtilization' in clientOrOptions) {
    client = context.client;
    utilizationOverride = clientOrOptions.customUtilization;
  } else {
    client = context.client;
    utilizationOverride = customUtilization;
  }

  const [supplyRate, setSupplyRate] = useState<BigNumber | null>(null);
  const [supplyRateAPR, setSupplyRateAPR] = useState<number | null>(null);
  const [supplyRateAPY, setSupplyRateAPY] = useState<number | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<Error | null>(null);

  // Get the current utilization if no custom utilization is provided
  const { utilization: currentUtilization, loading: utilizationLoading, error: utilizationError } = 
    useGetUtilization(client ? client : undefined);

  const fetchSupplyRate = useCallback(async () => {
    if (!client || !client.isInitialized()) {
      setError(new Error('CometClient is not initialized'));
      setLoading(false);
      return;
    }

    // If we're using current utilization and it's still loading, wait
    if (!utilizationOverride && utilizationLoading) {
      return;
    }

    // If there was an error fetching utilization, propagate it
    if (!utilizationOverride && utilizationError) {
      setError(utilizationError);
      setLoading(false);
      return;
    }

    try {
      setLoading(true);
      setError(null);

      const comet = client.getComet();
      if (!comet) {
        throw new Error('Comet contract is not available');
      }

      // Use custom utilization if provided, otherwise use current utilization
      const utilizationToUse = utilizationOverride || currentUtilization;
      
      if (!utilizationToUse) {
        throw new Error('Utilization is not available');
      }

      // Get the supply rate for the given utilization
      const supplyRateBN = await comet.getSupplyRate(utilizationToUse);
      setSupplyRate(supplyRateBN);

      // Convert to APR (Annual Percentage Rate)
      // Supply rate is returned as a per-second rate scaled by 1e18
      const secondsPerYear = 60 * 60 * 24 * 365;
      const supplyRatePerSecond = parseFloat(CometClient.formatUnits(supplyRateBN, 18));
      const apr = supplyRatePerSecond * secondsPerYear * 100; // Convert to percentage
      setSupplyRateAPR(apr);

      // Convert to APY (Annual Percentage Yield) with compounding
      const apy = (Math.pow(1 + supplyRatePerSecond, secondsPerYear) - 1) * 100; // Convert to percentage
      setSupplyRateAPY(apy);
    } catch (err) {
      console.error('Error fetching supply rate:', err);
      setError(err instanceof Error ? err : new Error('Unknown error fetching supply rate'));
    } finally {
      setLoading(false);
    }
  }, [client, utilizationOverride, currentUtilization, utilizationLoading, utilizationError]);

  useEffect(() => {
    fetchSupplyRate();
  }, [fetchSupplyRate]);

  return {
    supplyRate,
    supplyRateAPR,
    supplyRateAPY,
    loading,
    error,
    refetch: fetchSupplyRate
  };
} 