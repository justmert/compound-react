/**
 * Tests for useGetFactorScale hook
 * 
 * Testing approach:
...
import { useGetFactorScale } from '../useGetFactorScale';
...
// Mock CometClient
const mockFactorScale = jest.fn().mockResolvedValue(BigNumber.from('1000000000000000000')); // 1e18

const mockCometContract = {
  factorScale: mockFactorScale,
};
...
describe('useGetFactorScale', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockClient.isInitialized.mockReturnValue(true);
    mockClient.getComet.mockReturnValue(mockCometContract);
    mockFactorScale.mockResolvedValue(BigNumber.from('1000000000000000000'));
  });

  it('should fetch factor scale correctly', async () => {
    const client = new CometClient({} as any, 1);
    
    const { result } = renderHook(() => useGetFactorScale(client), { wrapper });
    
    // Initial state should be loading
    expect(result.current.loading).toBe(true);
    expect(result.current.error).toBeNull();
    expect(result.current.factorScale).toBeNull();
    ...
    expect(mockFactorScale).toHaveBeenCalled();
    
    // Verify hook output
    expect(result.current.error).toBeNull();
    expect(result.current.factorScale).toEqual(BigNumber.from('1000000000000000000'));
  });
  ...
  it('should handle refetch correctly', async () => {
    ...
    mockFactorScale.mockResolvedValue(BigNumber.from('2000000000000000000'));
    ...
    // Verify client interaction
    expect(mockFactorScale).toHaveBeenCalledTimes(2);
    
    // Verify hook output
    expect(result.current.loading).toBe(false);
    expect(result.current.error).toBeNull();
    expect(result.current.factorScale).toEqual(BigNumber.from('2000000000000000000'));
  ...
  it('should handle errors correctly', async () => {
    ...
    // Mock an error
    const mockError = new Error('Failed to fetch factor scale');
    mockFactorScale.mockRejectedValue(mockError);
  ...
  it('should handle missing client', async () => {
    const { result } = renderHook(() => useGetFactorScale(null), { wrapper });
    ...
    // Verify client interaction
    expect(mockFactorScale).not.toHaveBeenCalled();
  });
}); 